...
from src.concepts import PERIODIC_SCALES, LLM_KEY_PERIODIC
from src.embeddings import load_glove, get_vectors, extract_llm_activations

RESULTS_DIR = Path(__file__).resolve().parents[1] / "results"

TEMPLATES = {
    "months":       "The month of the year is {word}",
    "days_of_week": "The day of the week is {word}",
    "compass":      "The compass direction is {word}",
}

# ── GloVe analysis ────────────────────────────────────────────────────────────

def analyze_scale(scale_name, words, W, word_to_idx):
    found_words, vecs = get_vectors(words, W, word_to_idx)
    ...
    phases = [idx_map[w] / len(words) for w in found_words]
...
def run_glove():
    ...
    scale_results = {}
    for name, words in PERIODIC_SCALES.items():
        try:
            scale_results[name] = analyze_scale(name, words, W, word_to_idx)
...
def run_llm(model_name, device):
    ...
    for llm_key, scale_key in LLM_KEY_PERIODIC.items():
        words = PERIODIC_SCALES[scale_key]
    ...
    if all_acts:
        npz_path = RESULTS_DIR / "llm_periodic.npz"
...
def main():
    parser = argparse.ArgumentParser(description="Periodic BC / cyclic scales")
    ...
    output = {
        "experiment": "periodic_bc",
        "theory":     "Periodic BC → ellipse in PC plane from cos/sin harmonics",
        "glove":      run_glove(),
    }
    ...
    path = RESULTS_DIR / "periodic.json"
